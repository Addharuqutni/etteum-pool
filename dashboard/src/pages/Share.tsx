...
export default function Share() {
  ...
  if (error) {
    return (
      <div className="flex h-screen items-center justify-center font-mono text-[13px] text-[var(--muted-foreground)]">
        {error}
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-4 px-4 py-10">
      <div className="rise">
        ...
        {data.description && (
          <p className="mt-1 font-mono text-[12px] text-[var(--muted-foreground)]">{data.description}</p>
        )}
      </div>
      <Card className="rise" style={{ ["--d" as string]: "40ms" }}>
        <CardContent className="space-y-1 pt-4 font-mono text-[12px] tabular-nums">
      ...
      {snippets.map((s, i) => (
        <Card key={s.id} className="rise overflow-hidden" style={{ ["--d" as string]: `${80 + i * 40}ms` }}>
          ...
          <pre className="max-h-64 overflow-auto border-t border-[var(--border)] bg-[var(--sunken)] px-4 py-3 font-mono text-[11px] leading-relaxed text-[var(--muted-foreground)]">{s.text}</pre>
        </Card>
      ))}
