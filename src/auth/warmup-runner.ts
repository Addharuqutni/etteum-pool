...
function mergeWarmupMetadata(account: Account, health: ProviderHealthResult) {
  ...
  // Always preserve the *server's* view of the quota in metadata so the
  // dashboard can surface drift between what the provider reports and what
  // we mirror into the account columns.
...
function mapHealthToAccountUpdate(account: Account, health: ProviderHealthResult): AccountWarmupUpdate {
  ...
  if (health.quota) {
      ...
      // provider already wrote into the DB instead of clobbering it with the sentinel.
...
function messageFor(result: WarmupResult, account: Account): string {
...
export async function warmupAccount(account: Account): Promise<WarmupResult> {
  ...
  const log = addAuthLog({
    ...
    provider: account.provider,
    step: health.kind,
    message: messageFor(result, account),
